      ...
      <div className="mx-auto max-w-5xl">
        <FadeIn>
            ...
            <div className="space-y-8">
              <div>
                <h2 className="font-mono text-sm text-cyan-400">
                  <span className="text-slate-600">$</span> The Challenge &amp; Solution
                </h2>
              ...
              {project.features && project.features.length > 0 ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  {project.features.map((feature, index) => (
                    ...
                    >
                      <span className="text-xl">{featureIcons[index % featureIcons.length]}</span>
            ...
            <div className="space-y-6">
              {project.specs && project.specs.length > 0 ? (
                <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-6">
                  <h2 className="font-mono text-xs uppercase tracking-[0.2em] text-slate-600">Project Info</h2>
                  <dl className="mt-4 space-y-3">
                    {project.specs.map((spec) => (
                      <div key={spec.label} className="flex items-baseline justify-between gap-4 text-sm">
                        <dt className="text-slate-500">{spec.label}</dt>
                        <dd className="text-right font-mono text-slate-200">{spec.value}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
          ...
          {project.deepDive && project.deepDive.length > 0 ? (
            <div className="mt-12 rounded-xl border border-slate-800 bg-slate-900/30 p-8">
              <h2 className="font-mono text-sm text-cyan-400">
                <span className="text-slate-600">$</span> Technical Deep Dive
              </h2>
